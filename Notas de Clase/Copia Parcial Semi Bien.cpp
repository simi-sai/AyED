...
void Seccion::agregarFruta(int c) {
  ...
  while (frutasRestantes > 0 && cantidadFruta <= Max) {
    ...
    if (frutasFaltantes >= frutasRestantes) {
...
/*
void Seccion::sacarFruta(int c) {
    int frutaRestante = c;
    int cajonesRestantes = (int)ceil((double)c / 20.0);
    int cajonesSacados = 0;
    int cajonesEnPila = 0;
    
    // Iterar a través de las pilas, comenzando desde la última
    for (int i = 2; i >= 0; i--) {
        Pila<Cajon *> &pila = cajonesApilados[i];
        cajonesEnPila = pila.pilavacia() ? 0 : 1;
        
        while (!pila.pilavacia() && cajonesRestantes > 0) {
            Cajon *cajon = pila.tope();
            int frutaEnCajon = cajon->getCantidad();
            int frutaSacada = min(frutaRestante, frutaEnCajon);
            frutaRestante -= frutaSacada;
            frutaEnCajon -= frutaSacada;
            cajon->setCantidad(frutaEnCajon);
    ...
    cantidadFruta -= c;
    cantidadCajones -= cajonesSacados;
}
*/
void Seccion::sacarFruta(int c) {
    int cantidadSacada = 0;
    int cantidadRestante = c;
    Pila<Cajon*> pilaActual = this->cajonesApilados[pilaId];

    while (!pilaActual.pilavacia() && cantidadRestante > 0) {
        Cajon* cajonActual = pilaActual.tope();
        int cantidadCajon = cajonActual->getCantidad();
        if (cantidadCajon <= cantidadRestante) {
            pilaActual.desapilar();
            cantidadSacada += cantidadCajon;
            cantidadRestante -= cantidadCajon;
            cantidadFruta -= cantidadCajon;
            cantidadCajones--;
            //delete cajonActual;
        }
        else {
            cajonActual->setCantidad(cantidadCajon - cantidadRestante);
            cantidadSacada += cantidadRestante;
            cantidadFruta -= cantidadSacada;
            cantidadRestante = 0;
        }
    }

    if (pilaActual.pilavacia()) {
        pilaId--;
        if (pilaId >= 0) {
            sacarFruta(cantidadRestante);
        }
    }
}
