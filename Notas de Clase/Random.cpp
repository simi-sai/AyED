...
//-------------------------------------------------------
class Tiempo { // Habria q ver si anda, creo q como no tiene milisegundos tonces
               // si los metemos al mismo tiempo (usando main en vez de el
               // metodo menu) no detecta diferencia
private:
  time_t currentTime;
  tm *localTime;
  int anio;
...
public:
  Tiempo() {
    ...
    suma = anio + mes + dia + hora * 3600 + min * 60 + sec;
  }
...
private:
  ...
  void listar(Nodo<Post> *&aux,
              int id); // met priv para encolar comentarios de un post
...
public:
  ...
  void Votacion(int post, int com, int vot) {
    VotacionAux(raiz, post, com, vot);
  }
...
void Arbol::MostrarLista() {
  Cola<Post> aux = new Cola<>();  //revisar
...
void Arbol::Stats(Nodo<Post> *&aux, int post) {
  ...
  if (aux->dato.getTipo() == 2) {
    ...
    if (aux->der != NULL) {
      ...
      if (idMaxRat != 0) {
        cout << "El comentario con mejor votacion es el " << idMaxRat << " con "
             << MAXRating << " votos." << endl;
      } else {
      ...
      if (Veces > 1) {
        cout << "\nEl usuario que mas veces participo es " << Participe
             << " con un total de " << Veces << " comentarios en el mismo post"
             << endl;
      } else {
...
public:
  ...
  void ColaFinal();
  void Ordenamiento(){ordenaQS(arreglo,1,3);}
};
...
void GestionPost::ColaFinal() {
  ...
  for(int i = 0; i < size; i++)
    {
      cout << arreglo[i].getTiempo().getSuma() << endl;
    }
}
...
int main() {
  ...
  gestionPost->agregarComentario(1, "Manolito",
                                 "Comentario al POST 1");              // COM 1
  ...
  */
  ...
  gestionPost->ColaFinal();
  gestionPost->Ordenamiento();
  // gestionPost.MostrarPosts();
  /*
