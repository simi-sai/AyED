...
public:
  ...
  int getCantidad() {if(this == NULL){return 0;} else{ return cantidad; };};
  void setCantidad(int c) {if(this == NULL){this->cantidad = 0;} else{ this->cantidad = c; };};
...
void Seccion::agregarFruta(int c) {
  ...
  if (this->getCantFruta() + c <= Max) {
    if (cajonesApilados[pilaId].pilavacia() || cajonesApilados[pilaId].tope()->estaCompleto()) {
      ...
      // cajón a la pila
      if (cantidadCajones <= (pilaId+1)*10) {
...
int main() {
  ...
  seccion_dulce->agregarFruta(230);
