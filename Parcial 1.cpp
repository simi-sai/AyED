...
public:
  ...
  int getCantFruta();
  int getCantFruta2(){return cantidadCajones * 19 + cantidadFruta;};
};
...
void Seccion::agregarFruta(int c) {
  ...
  if (this->getCantFruta() + c <= 200) {
    ...
    if (c > cantidadFaltante) {
      ...
      cajonesApilados.tope()->setCantidad(
          cajonesApilados.tope()->getCantidad() + c);
      cantidadFruta += c;
    }
  } else {
    cajonesApilados.tope()->setCantidad(c);
    cout << "No se puede almacenar mas fruta" << endl;
  }
}
...
void Seccion::sacarFruta(int cant) {
  if (cajonesApilados.pilavacia()) {
    ...
    if (cant >= 20) {
      if (this->cajonesApilados.tope()->getCantidad() == 20) {
...
void Seccion::imprimirPila() {
  ...
  while (!this->cajonesApilados.pilavacia()) {
    ...
    pilaAux.apilar(this->cajonesApilados.tope());
    this->cajonesApilados.desapilar();
  }
  this->cajonesApilados = pilaAux;
}
...
int main() {
  ...
  seccion_acida->agregarFruta(155);
  seccion_neutra->agregarFruta(120);

  cout << "CANTIDAD FRUTA: "<< seccion_acida->getCantFruta();
